...
func (_m *ClassService) CreateClass(studioClass *c.NewClass) error {
	ret := _m.Called(studioClass)

	var r0 error
	if rf, ok := ret.Get(0).(func(*c.NewClass) error); ok {
		r0 = rf(studioClass)
	} else {
		r0 = ret.Error(0)
	}
...
func (_m *ClassService) FindClassByName(name string) (*c.Class, error) {
	ret := _m.Called(name)
...
func (m *ClassService) FindAllClasses() (c.ClassesList, error) {
	ret := m.Called()
